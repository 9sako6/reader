...
The crate is compiled as both an `rlib` for native tests and a `cdylib` for the browser WebAssembly build. Browser code supplies the units, figures, and flow produced by the existing TypeScript Engine and Extractor. Rust returns serializable state transitions and effects; the viewer shell performs DOM work and executes timers.

Native model-based tests use the pinned Quint Connect release and the specification under `spec/`. The browser wrapper uses the wasm-bindgen exports generated from this same crate.

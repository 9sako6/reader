...
import type { PageScreen, ReaderViewHandlers } from "./types";

export function orderedPageChildren(model: PageScreen, handlers: ReaderViewHandlers): ReactNode[] {
  ...
  const figures = model.figures.map((figure, figureIndex) => ({ kind: "figure" as const, offset: figure.sourceOffset, order: figureIndex, value: <Figure key={`figure-${figure.sourceOffset}-${figureIndex}`} figureView={{ figure, figureIndex, status: "ready", brightness: figure.kind === "mermaid" ? "revealed" : "dimmed" }} handlers={handlers} page /> }));

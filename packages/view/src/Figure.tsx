...
export function Figure({ figureView, handlers, page }: { figureView: ReaderFigureView; handlers: ReaderViewHandlers; page: boolean }): ReactElement {
  ...
  const failed = status === "failed" || (page && textAssetFailed);
  const revealed = page ? textRevealed : status !== "failed" && figureView.brightness === "revealed";
  ...
  const imageSurface = !codeFallback && !failed ? (
    <button
      ...
      onClick={() => page ? setTextRevealed((value) => !value) : handlers.toggleFigureBrightness(figureIndex)}
    ...
    >
      <img
        ...
        decoding="async"
        loading={page ? "lazy" : undefined}
        data-reader-source={page ? figure.src : undefined}
        onLoad={page || loadToken === null ? undefined : () => handlers.figureLoad(figureIndex, loadToken)}
        onError={page ? () => setTextAssetFailed(true) : loadToken === null ? undefined : () => handlers.figureError(figureIndex, loadToken)}
        ref={(element) => {
  ...
  return (
    <figure
      ...
      data-reader-page-figure={page ? "true" : undefined}
      className={page ? "article-figure" : "spot-figure"}
      onClick={(event) => {

...
function button(label: string, onClick: () => void, extra: Record<string, unknown> = {}): ReactElement {
  return createElement("button", {
    type: "button",
    "aria-label": label === "続きを読む" ? label : undefined,
...
function renderLoading(model: Extract<ReaderViewModel, { kind: "loading" }>, handlers: ReaderViewHandlers): ReactElement {
  ...
  if (model.slow) {
    ...
    children.push(button("中止", handlers.cancel, { key: "cancel", className: model.mobile ? "launch-cancel" : undefined, position: "absolute", left: "50%", bottom: "32px", transform: "translateX(-50%)" }));
...
function renderMinimap(model: Extract<ReaderViewModel, { kind: "rsvp" }>): ReactElement | null {
  ...
  return createElement("aside", { "data-reader-minimap": "true", style: { minWidth: "0", height: "100%", display: "flex", flexDirection: "column", gap: "16px", padding: "16px 0", color: "rgba(255,255,255,0.62)" }, children: [
    createElement("div", { key: "title", style: { fontSize: "13px" }, children: "記事の構成" }),
    createElement("nav", { key: "nav", style: { minHeight: "0", overflow: "auto", display: "flex", flexDirection: "column", gap: "4px" }, children: model.headings.map((heading, index) => createElement("button", { key: `${index}-${heading.text}`, type: "button", "aria-current": index === model.activeHeadingIndex ? "location" : "false", style: { ...buttonStyle, minHeight: "36px", padding: "4px 8px", borderRadius: "8px", background: index === model.activeHeadingIndex ? "rgba(255,255,255,0.12)" : "transparent", textAlign: "left", fontSize: "13px", color: "inherit" }, children: heading.text })) }),
  ] });
}
...
function renderBlock(block: ReaderBlock, index: number): ReactElement {
  const tag = blockTag(block);
  const sentenceSpans = globalThis.Engine.splitSentenceSpans(block.text, "ja");
...
function orderedTextChildren(model: Extract<ReaderViewModel, { kind: "text" }>, handlers: ReaderViewHandlers): ReactNode[] {
  const blocks = model.blocks.map((block, index) => ({ kind: "block" as const, offset: block.start, value: renderBlock(block, index) }));
...
function renderDesktopRsvp(model: Extract<ReaderViewModel, { kind: "rsvp" }>, handlers: ReaderViewHandlers): ReactElement {
  const current = model.figure
    ? renderFigure(model.figure, handlers, false)
    : createElement("div", { "data-reader-unit": "true", "data-reader-position-kind": "text", "data-source-start": model.unit ? String(model.unit.start) : "0", "data-source-end": model.unit ? String(model.unit.end) : "0", "aria-live": "off", "aria-atomic": "false", style: { position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", width: "min(100%, 640px)", maxWidth: "calc(100% - 32px)", height: "1.35em", boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 12px", borderRadius: "12px", fontSize: "clamp(36px, 4.5vw, 64px)", fontWeight: "600", lineHeight: "1.35", textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", overflowWrap: "normal", wordBreak: "keep-all" }, children: model.unit?.text || "" });
  const transport = model.figure ? button("続きを読む", handlers.resumeFigure, { key: "resume", minWidth: "88px" }) : iconButton(model.playing ? "一時停止" : "再生", handlers.togglePlayback, { fontSize: "20px" });
  return createElement("div", { "data-reader-stage": "true", className: "rsvp-view", style: { position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", width: "min(980px, calc(100% - 48px))", height: "calc(100% - 48px)", display: "grid", gridTemplateColumns: model.headings.length > 0 ? "280px minmax(0, 1fr)" : "minmax(0, 1fr)", columnGap: model.headings.length > 0 ? "32px" : "0", alignItems: "stretch" }, children: [renderMinimap(model), createElement("main", { key: "main", style: { position: "relative", minWidth: "0", height: "100%" }, children: [
    ...
    createElement("div", { key: "controls", style: { position: "absolute", left: "50%", bottom: "8px", transform: "translateX(-50%)", width: "min(100%, 264px)", minHeight: "56px", display: "grid", gridTemplateColumns: "1fr 56px 1fr", alignItems: "center" }, children: [iconButton("1文戻る", handlers.previousSentence), transport, createElement("span", { key: "spacer" })] }),
